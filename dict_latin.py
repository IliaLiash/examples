...
s = sorted(set(' '.join(list1).replace(',','').split())) #составляем множество из слов, для которых требуется перевод латиницы

d2={}
for element in s:
    for k,v in d.items():
        if str(element) in str(v):  #если элемент в строковом представлении значения словаря (значение - список слов)
            if element in d2.keys():
                d2[element] += ', ' + k # дописываем к значению, если уже ключ-элемент есть в словаре
            else:
                d2[element] = k #иначе создаем
